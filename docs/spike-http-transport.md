...
6. **Session-auth binding (defense in depth).** *Caught by gemini-code-assist[bot] in PR #4 review.* The spike looks up transport by `Mcp-Session-Id` alone, without verifying that the current Bearer token matches the auth that initialized the session. Actual blast radius is narrow — every tool call constructs a fresh `BugSpotterClient` from the current request's auth, so upstream calls don't leak User A's key to User B. But the SDK Server/transport instance holds session-bound state from the original `initialize` (clientInfo, capabilities, MCP session settings), and an attacker who guesses or sniffs a sessionId could inherit that state. Production must bind `(sessionId, sessionHash)` at session creation and reject lookups where the auth doesn't match. I missed this in the spike; credit to Gemini's auto-review.
7. **Per-request client / per-session Server (efficiency).** Spike instantiates a new `BugSpotterClient` (and a fresh `Server`) on every tool call / session. For moderate load this is fine, but a single shared `Server` (handlers read auth from `extra`) plus a session-scoped client pool would cut overhead. *Also flagged by Gemini in the PR #4 review.*
