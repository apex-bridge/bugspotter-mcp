...
|---|---|---|---|
| `list_bugs` | Список багов проекта (только id, title, status, priority, даты) — для обзора и сортировки | — (если задан `BUGSPOTTER_DEFAULT_PROJECT` или заголовок `X-Project-Id`) | `status`, `priority`, `from_date`, `to_date`, `limit` (до 100) |
...
| Детали бага | «Покажи баг f8278dd4-1b79-4383-a39e-51aab5c2f8ae целиком» |
| Проверка дубликата | «Есть ли похожие баги на баг … ?» |
